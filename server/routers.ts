    ...
    const [macroDataResult, worldBankResult, imfDataResult, refinedTavilyResult,
      ...
      shouldFetchEurLex(taskDescription + " " + gptStep1Output)
        ? Promise.resolve(searchEuRegulations(taskDescription.slice(0, 100)))
            ...
            .catch(() => "")
        : Promise.resolve(""),
      // GLEIF：全球 LEI 法人识别码/法人结构/母子公司关系（检测到跨国公司/LEI/法人结构关键词时触发）
      shouldFetchGleif(taskDescription + " " + gptStep1Output)
        ? getCompanyLeiInfo(taskDescription.slice(0, 100))
...
%%FOLLOWUP%%请问下一个追问问题？%%END%%`;
    ...
    // 法律与监管类
    if (courtListenerMarkdown) apiSources.push({ name: "CourtListener", category: "法律监管", icon: "⚖️", description: "美国法院诉讼/判决历史" });
    if (congressMarkdown) apiSources.push({ name: "Congress.gov", category: "法律监管", icon: "🏹", description: "美国立法动态/法案进展" });
    if (eurLexMarkdown) apiSources.push({ name: "EUR-Lex", category: "法律监管", icon: "🇪🇺", description: "欧盟法规/MiCA/GDPR/DORA" });
    if (gleifMarkdown) apiSources.push({ name: "GLEIF", category: "法律监管", icon: "🏛️", description: "全球 LEI 法人识别码/法人结构" });
