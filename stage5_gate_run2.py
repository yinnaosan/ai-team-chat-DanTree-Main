#!/usr/bin/env python3
"""
Phase 4C Stage 5 — Semantic Gate Round 2 (post-CS-H3 patch)
Queries NEW samples only (id > 1530064, i.e., post-Stage4 batch)
Uses updated CONFIDENCE_LEVEL regex (Claude FINAL_REGEX)
"""
...
BASELINE_MSG_ID = 1530064  # Round 2: only new samples
...
print(f"Round 2 samples (id > {BASELINE_MSG_ID}): {len(samples)}")
if len(samples) < 10:
    print(f"WARNING: only {len(samples)} samples. Running with available data.")

# ── Semantic Gate rules (Claude FINAL_REGEX for CONFIDENCE_LEVEL) ─────────────
...
CONFIDENCE_LEVEL    = re.compile(
    r"高置信度|中置信度|低置信度"
    r"|置信度.{0,5}[高中低]|置信度.{0,5}(?:medium|high|low)"
    r"|[高中低].{0,4}置信"
    r"|信心为[高中低]|信心为(?:medium|high|low)"
    r"|信心.{0,4}[高中低]"
    r"|信心评级为[高中低]|信心评级为(?:medium|high|low)"
...
print("═" * 55)
print(f"  ROUND 2 FINAL: {'READY for integration ✓' if all_pass else 'NOT READY ✗'}")
print("═" * 55)
