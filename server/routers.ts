          ...
          json_schema: {
            ...
            schema: {
              ...
              properties: {
                ...
                risks: {
                  type: "array",
                  description: "风险项，每条必须绑定证据",
                  items: {
                    type: "object",
                    properties: {
                      description: { type: "string", description: "风险描述，必须针对当前标的具体量化（如「NVDA: AI芯片出口管制→中国收入占比40%面临风险，潜在收入损失$X亿」），禁止通用描述" },
                      magnitude: { type: "string", description: "风险幅度：high/medium/low" },
                      citations: { type: "array", items: { type: "string" }, description: "支撑此风险的 fact key" },
                    },
                    required: ["description", "magnitude", "citations"],
                    additionalProperties: false,
                  },
                },
...
%%DELIVERABLE%%
{
  ...
  "confidence": "high|medium|low",
  "key_evidence": [
    "看多(BULL)理由1：无论结论方向如何，此处必须填写支持看多的论点（如业务增长/估值安全边际/技术领先），禁止填写看空内容",
    "看多(BULL)理由2：不同维度的看多论点（业务/技术/市场/竞争四选一）"
  ],
  "reasoning": ["推理链1", "推理链2"],
  "counterarguments": [
    "看空(BEAR)理由1：无论结论方向如何，此处必须填写支持看空的论点（如高估值/宏观风险/竞争加剧），禁止填写看多内容",
    "看空(BEAR)理由2：不同维度的看空论点（估值/业务/宏观/情绪四选一）",
    "看空(BEAR)理由3：最强看空论点：如果只有一个理由能推翻看多结论，是什么"
  ],
  "risks": [{"description": "风险描述（必须针对当前标的具体量化，如「NVDA: 出口管制→中国收入-40%，EPS影响约-$1.2/股」），禁止通用描述", "magnitude": "high|medium|low"}],
