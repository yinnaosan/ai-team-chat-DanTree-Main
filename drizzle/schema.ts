...
export const analysisMemory = mysqlTable("analysis_memory", {
  ...
  verdict: varchar("verdict", { length: 500 }).notNull().default(""),
  confidenceLevel: varchar("confidenceLevel", { length: 40 }).notNull().default(""),
  ...
  outputMode: varchar("outputMode", { length: 40 }).notNull().default(""),
...
export const watchItems = mysqlTable("watch_items", {
  ...
  /** B4: persisted previous PE ratio for valuation_shift delta comparison across restarts */
  lastValuation:     decimal("last_valuation", { precision: 8, scale: 4 }),   // nullable
  createdAt:         bigintCol("created_at", { mode: "number" }).notNull(),
