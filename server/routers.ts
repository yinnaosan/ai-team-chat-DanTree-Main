              ...
              properties: {
                ...
                risks: {
                  ...
                  items: {
                    type: "object",
                    properties: {
                      description: { type: "string", description: "风险描述，必须量化（如利率上升100bp→估值压缩8-12%）" },
...
%%DELIVERABLE%%
{
  ...
  "confidence": "high|medium|low",
  "key_evidence": ["证据1（来源，数值）", "证据2"],
  "reasoning": ["推理链1", "推理链2"],
  "counterarguments": ["反驳论点1"],
  "risks": [{"description": "风险描述", "magnitude": "high|medium|low"}],
  "next_steps": ["建议行动1", "建议行动2"]
}
%%END_DELIVERABLE%%

%%DISCUSSION%%
{
  "key_uncertainty": "当前最大的不确定性",
  "weakest_point": "本次分析最薄弱的环节",
  "alternative_view": "与本结论相反的合理观点",
  "follow_up_questions": ["追问1", "追问2", "追问3"],
  "exploration_paths": ["延伸研究方向1", "延伸研究方向2"]
}
%%END_DISCUSSION%%
        ...
        try {
          const classifyResp = await invokeLLMWithRetry({
            messages: [
              {
                role: "system",
                content: `你是一个记忆分类助手。根据任务描述和摘要，将记忆分类为以下四种类型之一：
